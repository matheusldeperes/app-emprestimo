    ...
    try:
        ...
        <html>
            <body style="font-family: Arial; font-size: 12px;">
                ...
                <p><strong>Data/Hora de Entrada:</strong> {datetime.now(ZoneInfo('America/Sao_Paulo')).strftime('%d/%m/%Y %H:%M')}</p>
...
def salvar_pdf_em_servidor(arquivo_pdf_bytes, placa, cliente_nome, consultor_nome, diretorio_destino):
    ...
    try:
        ...
        with open(caminho_arquivo, "wb") as arquivo:
            arquivo.write(arquivo_pdf_bytes)

        return True, caminho_arquivo
    except Exception as e:
        print(f"Erro ao salvar PDF em servidor local: {e}")
        st.error(f"Erro ao salvar PDF em servidor local: {e}")
        return False, None
...
def gerar_pdf_bytes(logo_path, nome_cliente, placa, observacoes, consultor, numero_os, data_hora, fotos):
    ...
    pdf.cell(0, 5, f"Data de geração: {datetime.now(ZoneInfo('America/Sao_Paulo')).strftime('%d/%m/%Y %H:%M')}", align="C")
...
data_hora_checklist = datetime.now(ZoneInfo('America/Sao_Paulo')).strftime('%d/%m/%Y %H:%M')
...
# Simulação de envio do PDF para pasta de servidor local
with st.expander("🗂️ Simulação: salvar PDF em pasta de servidor local", expanded=False):
    st.caption("Use esta seção para simular o envio do PDF para uma pasta de servidor local.")
    salvar_no_servidor = st.checkbox(
        "Salvar PDF em pasta do servidor (simulação)",
        value=True
    )
    pasta_servidor = st.text_input(
        "Pasta de destino (caminho local)",
        value=os.path.join("servidor_local", "entrada")
    )
...
if not st.session_state.finalizado:
    ...
    if st.button("✅ Finalizar Check-in e Enviar", use_container_width=True, disabled=not botao_liberado, type="primary"):
        with st.spinner("Gerando PDF e enviando emails..."):
            ...
            # Salvar PDF em pasta local (simulação de servidor)
            if salvar_no_servidor:
                sucesso_salvar, caminho_salvo = salvar_pdf_em_servidor(
                ...
                if sucesso_salvar and caminho_salvo:
...
if st.session_state.finalizado:
    ...
    st.download_button(
        ...
        file_name=f"CheckIn_Oficina_{placa_veiculo.upper()}_{datetime.now(ZoneInfo('America/Sao_Paulo')).strftime('%Y%m%d_%H%M')}.pdf",
